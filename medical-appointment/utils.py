...
def evidence_interval(start: Any, end: Any) -> Span | None:
...
def gold_evidence(row: dict[str, str]) -> Span | None:
...
def temporal_iou(ground_truth: Span, prediction: Span | None) -> float:
    ...
    union = max(ground_truth_end, prediction_end) - min(
        ground_truth_start, prediction_start
    )
...
def mean_temporal_iou(
    labels: list[int],
    ground_truths: list[Span | None],
    predictions: list[Span | None],
) -> float:
    ...
    scores = [
        ...
        for label, ground_truth, prediction in zip(
            labels, ground_truths, predictions, strict=True
        )
...
def validate_response(
    ...
    for position, answer in enumerate(response.answers):
        if not isinstance(answer, bool):
            raise ValueError(
                f"answers[{position}] must be a bool, got " f"{type(answer).__name__}."
            )
    ...
    for position, (start, end) in enumerate(
        zip(response.evidence_start, response.evidence_end, strict=True)
    ):
